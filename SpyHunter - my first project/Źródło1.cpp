...
//start new game
void resetVariables(int* frames, double* fpsTimer, double* fps, int* quit, double* worldTime, double* distance, double* etiSpeed)
{
	*frames = 0;
	*fpsTimer = 0;
	*fps = 0;
...
int main(int argc, char** argv) {
	...
	SDL_SetWindowTitle(window, "SpyHunter by Tomasz Sankowski 193363");
	...
	resetVariables(&frames, &fpsTimer, &fps, &quit, &worldTime, &distance, &etiSpeed);
	...
	while (!quit) {
		...
		fpsTimer += delta;
		if (fpsTimer > FPS_FREQUENCE) {
			fps = frames / FPS_FREQUENCE;
			frames = 0;
			fpsTimer -= FPS_FREQUENCE;
		};
